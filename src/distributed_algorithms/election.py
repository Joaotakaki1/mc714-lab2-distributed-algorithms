...
class BullyElection:
    ...
    async def start_election(self, reason: str = "manual") -> dict[str, object]:
        async with self._lock:
            ...
            self._history.append(ElectionEvent("start", reason))
        ...
        async with self._lock:
            self._history.append(ElectionEvent("wait-coordinator", "higher node answered OK"))
    ...
    async def handle_election_message(self, sender_id: str, reason: str) -> dict[str, object]:
        async with self._lock:
            self._history.append(ElectionEvent("receive-election", f"from node {sender_id}: {reason}"))
    ...
    async def handle_coordinator_message(self, sender_id: str, reason: str) -> dict[str, object]:
        async with self._lock:
            ...
            self._election_in_progress = False
            self._history.append(ElectionEvent("coordinator", f"leader is node {sender_id}: {reason}"))
    ...
    async def _become_leader(self, reason: str) -> None:
        async with self._lock:
            ...
            self._election_in_progress = False
            self._history.append(ElectionEvent("become-leader", reason))
            self._coordinator_received.set()
        payload = {"sender_id": self.node_id, "reason": reason}
        lower_peer_ids = [peer_id for peer_id in self._transport.peer_ids if int(peer_id) < int(self.node_id)]
        await self._transport.broadcast("/election/internal/coordinator", payload, lower_peer_ids)
